  ...
  return (
    <div className="flex flex-col min-h-screen bg-surface font-sans">
      ...
      <header className="flex items-center justify-between px-5 py-4 border-b border-outline-variant bg-surface-container-lowest">
        <button
          onClick={onBack}
          className="flex items-center gap-1 text-sm text-on-surface-variant min-h-[44px] px-2 cursor-pointer btn-press"
        >
      ...
      <div className="flex-1 flex flex-col px-5 py-6 gap-5 max-w-lg mx-auto w-full">
        <div>
          ...
          <h1 className="font-display text-xl font-bold text-on-surface mb-1">
            Which part of {topic} should we focus on?
          </h1>
          <p className="text-sm text-on-surface-variant">
            Pick the area you want to explain. One at a time.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2.5">
          {subtopics.map((sub) => (
            ...
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center ${selected === sub.title ? "bg-primary-fixed" : "bg-surface-container-low"}`}
              >
                <span
                  className={`material-symbols-outlined text-lg ${selected === sub.title ? "text-on-primary-fixed" : "text-on-surface-variant"}`}
                >
                  {sub.icon}
                </span>
              </div>
              <span
                className={`text-sm font-semibold leading-snug ${selected === sub.title ? "text-primary" : "text-on-surface"}`}
              >
                {sub.title}
              </span>
